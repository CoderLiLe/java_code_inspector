...
| 方法复杂度分析 | 检测圈复杂度过高的方法 |
| 空方法检测 | 检测空方法体 |
| 魔法数字检测 | 检测代码中的魔法数字（跳过注释和字符串中的数字） |
...
| 并行检查 | 多线程并发检查，大幅提升大型项目检查速度 |
| 环境变量配置 | 支持 `JAVA_INSPECTOR_*` 环境变量深度覆盖任意配置项 |
...
│       ├── hooks.py              # Git钩子安装
│       ├── inspector.py          # 核心检查引擎（含并行检查）
│       ├── models.py             # 数据模型（CodeIssue、CodeMetrics）
│       ├── reporter.py           # 报告生成器（5种格式）
│       ├── ci_cd.py              # CI/CD质量门禁
│       ├── alibaba_rules.py      # 阿里规约入口（委派到 alibaba_rules/ 包）
│       ├── sonarqube_rules.py    # SonarQube规则入口（委派到 sonarqube/ 包）
...
│       │   └── design.py         # 设计规约 (6 条)
│       └── sonarqube/            # SonarQube 规则检查器包（17个模块 + 基类）
│           ├── __init__.py       # 所有检查器导出 + get_checker_classes()
│           ├── base.py           # BaseSonarChecker 基类
...
│   ├── __init__.py
│   ├── test_config.json
│   ├── test_java_inspector.py    # 测试套件（312个测试用例）
│   └── test_file/
│       ├── GoodExample.java
│       └── TestExample.java
├── docs/                         # 文档目录
│   ├── ALIBABA_RULES.md          # 阿里规约规则总表（293条）
│   ├── JAVA_MANUAL.md            # 《Java开发手册(黄山版)》全文
...
# 并行检查（自定义线程数）
python -m java_inspector /path/to/project --workers 8

# 详细日志输出
python -m java_inspector /path/to/project --verbose

# 安静模式（仅错误信息）
python -m java_inspector /path/to/project --quiet
```
...
所有配置项均可通过 `JAVA_INSPECTOR_*` 前缀的环境变量覆盖，使用 `__`（双下划线）作为路径分隔符：
...
环境变量命名规则：`JAVA_INSPECTOR_<一级键>__<二级键>__<三级键>`，如 `JAVA_INSPECTOR_RULES__LINE_LENGTH__ENABLED=false`。
...
task codeQualityCheck(type: Exec) {
    commandLine 'python3', '-m', 'java_inspector', 'src/main/java', '--ci-cd', '--config', 'java_inspector_config.json'
...
ENTRYPOINT ["python3", "-m", "java_inspector"]
CMD ["--help"]
```
...
def run_code_inspection(project_path, config_path=None):
    ...
    cmd = ["python3", "-m", "java_inspector", project_path,
